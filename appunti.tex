...
\subsection{Building Atomic from Regular Registers}
...
\textbf{Algorithm:}
\begin{enumerate}
\item For each write, the writer writes $(v, ts)$ to each reader's private register.
\item Each reader reads from its private register.
\item If a reader receives multiple values, it keeps the one with the highest timestamp.
\end{enumerate}
...
\textbf{Algorithm:}
\begin{enumerate}
\item The writer sends $(v, ts)$ to all processes as in Read-One-Write-All.
\item When a reader performs a read, it:
  \begin{itemize}
  \item Gets the value $(v, ts)$ from a majority of processes.
  \item Sends this value back to all correct processes to ensure they all have at least this timestamp.
  \item Returns the value to the caller.
  \end{itemize}
\end{enumerate}

This extra ``write back'' ensures that all subsequent reads will see this timestamp or something newer, maintaining the ordering property.

\subsection{Read-Impose Write-Majority Algorithm}

For the crash-silent model (no perfect failure detection):

\textbf{Algorithm:}
\begin{enumerate}
\item The writer sends $(v, ts)$ to a majority using timestamps.
\item The reader:
  \begin{itemize}
  \item Queries a majority of processes to find the value with the highest timestamp.
  \item Sends this value back to a majority to ``impose'' the write.
  \item Returns the value.
  \end{itemize}
\end{enumerate}

The majority voting ensures that future reads will find this value because any new majority must overlap with the previous majority (intersection property).
